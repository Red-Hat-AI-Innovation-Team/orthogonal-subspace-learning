...
def collate_fn(batch, tokenizer, max_length=1024):
    ...
    for i in range(len(full_texts)):
        prompt_length = int(prompt_encodings["input_ids"][i].ne(tokenizer.pad_token_id).sum())
...
def load_model():
    ...
    model = LlamaForCausalLM.from_pretrained(model_name, config=config, device_map="auto")
